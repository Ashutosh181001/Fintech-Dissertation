"""
Main Detection Module (Confluent Cloud–ready)
Real-time anomaly detection pipeline for multiple crypto pairs
Supports: BTC/USDT, ETH/USDT, BNB/USDT, SOL/USDT, XRP/USDT

This version replaces kafka-python with confluent_kafka (librdkafka)
so it can connect reliably to Confluent Cloud using SASL_SSL.

Requirements (ensure these are in your image):
    confluent-kafka
    certifi
"""
...
from confluent_kafka import Consumer as CKConsumer, TopicPartition
...
class MultiSymbolAnomalyDetector:
    ...
    def __init__(self):
        ...
        self.is_model_fitted: Dict[str, bool] = {}
        self.history_dict: Dict[str, pd.DataFrame] = {}
        self.buffer_for_training: Dict[str, list] = {}
        self.last_anomaly_time: Dict[str, Dict[str, float]] = {}  # For cooldown tracking
    ...
    def _init_kafka_consumer(self) -> CKConsumer:
        try:
            kcfg = config.KAFKA_CONFIG
            conf = {
                "bootstrap.servers": kcfg["broker"],
                "group.id": kcfg["consumer_group"],
                "auto.offset.reset": kcfg.get("auto_offset_reset", "latest"),
                "enable.auto.commit": True,
                "client.id": "crypto-detector",

                # Essential for Confluent Cloud
                "security.protocol": "SASL_SSL",
                "sasl.mechanism": "PLAIN",
                "sasl.username": kcfg["sasl_plain_username"],
                "sasl.password": kcfg["sasl_plain_password"],
            ...
            # log a scrubbed view of what we're sending (no passwords)
            scrub = {**conf}
            scrub.pop("sasl.password", None)
            logging.getLogger(__name__).info("Consumer conf (scrubbed): %s", json.dumps(scrub, indent=2))

            consumer = CKConsumer(conf)
            consumer.subscribe([kcfg["topic"]])
            logging.getLogger(__name__).info("Kafka consumer (confluent_kafka) initialized & subscribed")
            return consumer
        except Exception as e:
            logging.getLogger(__name__).error(f"Failed to initialize Kafka consumer: {e}")
            raise
    ...
    def _safe_offset(self, off: int) -> int:
        return off if isinstance(off, int) and off >= 0 else 0

    def _sample_consumer_lag(self):
        """Compute simple group lag snapshot (min/avg/max) and log it using committed offsets vs high watermark."""
        try:
            parts: List[TopicPartition] = self.consumer.assignment()
            if not parts:
                return
            lags = []
            for tp in parts:
                # get high watermark (latest) and committed for group
                low, high = self.consumer.get_watermark_offsets(tp, timeout=5.0)
                committed = self.consumer.committed([tp], timeout=5.0)[0]
                high_off = self._safe_offset(high)
                committed_off = self._safe_offset(getattr(committed, 'offset', -1))
                lag = max(high_off - committed_off, 0)
                lags.append(lag)
            if lags:
    ...
    def run(self):
        ...
        while True:
            ...
            if msg is None:
                # periodic metrics flush even if idle
                self._sample_consumer_lag()
                self.benchmark.maybe_flush()
                continue
            if msg.error():
                logger.debug(f"Kafka message error: {msg.error()}")
                continue
